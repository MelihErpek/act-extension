            ...
            .then(data => {
                if (data) {
                    ...
                    chrome.storage.local.set({ rules: data }, function () {
                        ...
                        campaignSubmittedMessage.textContent = "Campaign Submitted: " + campaignName;
        ...
        chrome.storage.local.get('rules', function (data) {
            if (data.rules) {
                ...
                if (budgetRule) {
                    ...
                    var inputs2 = document.querySelectorAll('input[placeholder="Please enter amount"]');
                    budgetCampaign = true;
                    if (inputs.length > 0) {
                        budgetCampaign = true;
                    ...
                    if (bExceed) {
                        ...
                        var publishDiv = Array.from(document.querySelectorAll('div')).find(div => div.textContent.trim() === "Publish");
                ...
                if (data.rules[0].campaignName !== undefined) {
                    ...
                    let inputElement3 = document.querySelector('input[placeholder="Enter your campaign name here..."]');
                    if (inputElement3) {
...
function checkCampaignNameMatch(rules, cname) {
    ...
    if (adcamInput) {  // Element kontrolü yapılıyor.
...
function attachInputHandlers(rules) {
    if (googleBExceed) {
        var publishDiv = Array.from(document.querySelectorAll('span')).find(div => div.textContent.trim() === "Kampanyayı yayınla");
...
function checkCampaignLifetimeBudgetInput(Budget, BudgetType) {
    if (BudgetType === "Lifetime Bugdet") {
        const input = document.querySelector('input[id="bid-and-budget__total-budget"]');
        if (input) {
        ...
        const input2 = document.querySelector('input[id="bid-and-budget__daily-budget"]');
...
function lBExceed() {
    if (linkedinBExceed) {
        var publishDiv = Array.from(document.querySelectorAll('button')).find(div => div.textContent.trim() === "İleri");
        if(publishDiv){
